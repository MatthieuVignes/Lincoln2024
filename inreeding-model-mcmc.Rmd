...
We review our simple model for inbreeding at a locus and develop the code to perform several different types of Metropolis-Hastings updates, then augment the model to include latent variables that let us easily to Gibbs-sampling updates.

The following will install tidyverse and other packages we need before we get going if you don't have them. 
```{r}
...
```{r echo=FALSE}
knitr::include_graphics("inbreeding_dag.png")
```
