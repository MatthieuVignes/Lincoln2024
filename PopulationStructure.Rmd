...
```{r}
pacman::p_load(devtools, gaston, pegas, HardyWeinberg)
pacman::p_load_gh("jgx65/hierfstat", "jgx65/JGTeach")

```

1. We will start by doing in silico Buriâ's drosophila cages experiment. We will generate using the JGTeach::drift function the allelic frequency trajectories for a number of replicates, corresponding to the different cages:
...
par(mfrow=c(1,1))
```
1. Describe the resulting figure and compare it to Buriâ's results
	...
	4.	Very similar to Buriâ's experiment with drosophila
 - Describe the resulting figure and compare it to Buriâ's results
1.	trajectories of allele frequencies, starting with freq=0.5
2.	with lower pop size, fixation faster; with higher pop size, slower
...
another, in order to check the approximation Î² â‰ˆ t/2N . The 3 populations have diploid sizes
...
The approximatoin t/2N works only if t<N/2, the same with sim.genot.t or ms.
...
panels show F^{XY}st (or Î²^{XY} ) on top and population specific Fst at the bottom. Last, the
